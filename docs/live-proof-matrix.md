...
| List creation | `interspire_list_create_preview` / `interspire_list_create_apply` | Preview/apply | Preview finds the Interspire create form and source-derived `AddList` route; apply creates exactly one new list, then proves requested metadata on the new edit page, using a guarded post-create metadata save if Interspire ignored Bounce Email on create. | Missing form, stale plan, missing gates, wrong action route, duplicate new ids, field persistence mismatch, webhook/multi-select state loss. | Preview and apply on a disposable target list; reread list owner metadata and confirm local bounce polling was not enabled unless explicitly requested. |
