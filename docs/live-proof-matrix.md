...
| OCI send ledger preflight | `interspire_seed_send_apply` / `interspire_production_send_apply` `oci_ledger_preflight` | Send-adjacent gate | When `INTERSPIRE_REQUIRE_OCI_SEND_LEDGER=1`, the guarded send refuses before the final send form unless the configured private JSONL ledger contains the expected Interspire campaign/batch rows with recipient and trace keys. Output returns hashes and counts only. | Missing env path, missing preflight request, ledger campaign id not equal to the Interspire campaign being sent, wrong row count, missing recipient key, missing message/correlation key, invalid JSON row, raw campaign/recipient leakage, per-call path attempt. | Use a synthetic private ledger file and gated send request on a non-sending fixture path; prove the report blocks before admin send automation when ledger rows are absent or tied to another campaign and verifies only hashes/counts when present. |
